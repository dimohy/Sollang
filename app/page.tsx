...
export default function PlaygroundPage() {
  ...
  const pending = useRef(new Map<number, (result: CompilerResult) => void>());

  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      if (event.origin !== window.location.origin) return;
      if (event.data?.type === "sollang-compiler-ready") {
        setCompilerState("ready");
      } else if (event.data?.type === "sollang-compiler-failed") {
        setCompilerState("failed");
        setResult({
          success: false,
          output: "",
          diagnostics: `sollang: ${event.data.message}`,
          compileMilliseconds: 0,
          executeMilliseconds: 0
        });
      } else if (event.data?.type === "sollang-compile-result") {
        const resolve = pending.current.get(event.data.id);
        if (!resolve) return;
        pending.current.delete(event.data.id);
        resolve(JSON.parse(event.data.result) as CompilerResult);
      }
    };
  ...
  const run = useCallback(async () => {
    ...
    const iframe = document.querySelector<HTMLIFrameElement>("#compiler-runtime");
    if (!iframe?.contentWindow) return;

    setIsRunning(true);
    setResult(null);
    const id = ++requestId.current;
    const response = new Promise<CompilerResult>(resolve => pending.current.set(id, resolve));
    iframe.contentWindow.postMessage(
      { type: "sollang-compile-run", id, source: code },
  ...
  return (
    <main className="site-shell">
      <iframe
        id="compiler-runtime"
        className="compiler-frame"
        src="/compiler-0.2.260723/index.html"
        title="Sollang WebAssembly compiler"
      />
            ...
            <div className={`terminal ${result && !result.success ? "terminal-error" : ""}`}>
              ...
              {isRunning && (
                <div className="terminal-empty">
                  ...
                  <span>lexer → parser → semantic → VM</span>
                </div>
              )}
              {result && (
                <>
      ...
      <footer>
        ...
        <span>Compiler runtime: WebAssembly · No server round-trip</span>
      </footer>
    </main>
  );
}
