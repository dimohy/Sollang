...
public sealed class LexerSourceGenerator : IIncrementalGenerator
{
    private static readonly DiagnosticDescriptor InvalidSpec = new(
        id: "SOLLANGGEN001",
        title: "Invalid Sollang lexer specification",
        messageFormat: "{0}",
        category: "Sollang.Lexer",
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var specs = context.AdditionalTextsProvider
            .Where(static text => Path.GetFileName(text.Path).Equals("sollang.lexer", StringComparison.OrdinalIgnoreCase))
    ...
    string? Error)
{
    ...
    {
        var lexerType = "Sollang.Compiler.Lexing.Lexer";
...
internal static class LexerEmitter
{
    public static string Emit(LexerSpec spec)
    {
        ...
        var ns = lastDot >= 0 ? spec.LexerType.Substring(0, lastDot) : "Sollang.Compiler.Lexing";
        ...
        var hasCharacter = spec.Tokens.Any(static rule => rule.Pattern == "character_literal");
        ...
        {
            builder.AppendLine("                case '\\'':");
            builder.AppendLine("                    LexCharacter();");
            builder.AppendLine("                    break;");
        }
        ...
        if (hasCharacter)
        {
            builder.AppendLine("    private void LexCharacter()");
            builder.AppendLine("    {");
            builder.AppendLine("        var line = _line;");
            builder.AppendLine("        var column = _column;");
            builder.AppendLine("        Advance();");
            ...
            builder.AppendLine("        {");
            builder.AppendLine("            throw ErrorAt(line, column, \"unterminated character literal\");");
            builder.AppendLine("        }");
            builder.AppendLine();
            builder.AppendLine("        int scalar;");
            ...
            builder.AppendLine("                throw ErrorAt(line, column, \"unterminated character literal\");");
            builder.AppendLine("            }");
            builder.AppendLine("            scalar = Current switch");
            builder.AppendLine("            {");
            ...
            builder.AppendLine("                '\\'' => '\\'',");
            builder.AppendLine("                '\"' => '\"',");
            builder.AppendLine("                _ => throw ErrorAt(line, column, $\"unsupported character escape '\\\\{Current}'\")");
            builder.AppendLine("            };");
            builder.AppendLine("            Advance();");
            builder.AppendLine("        }");
            builder.AppendLine("        else if (char.IsHighSurrogate(Current))");
            builder.AppendLine("        {");
            builder.AppendLine("            var high = Current;");
            builder.AppendLine("            Advance();");
            builder.AppendLine("            if (IsAtEnd || !char.IsLowSurrogate(Current))");
            builder.AppendLine("            {");
            builder.AppendLine("                throw ErrorAt(line, column, \"character literal must contain one Unicode scalar\");");
            builder.AppendLine("            }");
            builder.AppendLine("            scalar = char.ConvertToUtf32(high, Current);");
            builder.AppendLine("            Advance();");
            builder.AppendLine("        }");
            builder.AppendLine("        else");
            builder.AppendLine("        {");
            builder.AppendLine("            if (char.IsLowSurrogate(Current) || Current == '\\'')");
            builder.AppendLine("            {");
            builder.AppendLine("                throw ErrorAt(line, column, \"character literal must contain one Unicode scalar\");");
            builder.AppendLine("            }");
            builder.AppendLine("            scalar = Current;");
            builder.AppendLine("            Advance();");
            builder.AppendLine("        }");
            builder.AppendLine();
            builder.AppendLine("        if (IsAtEnd || Current is '\\r' or '\\n')");
        ...
        builder.AppendLine("        var start = _index;");
        builder.AppendLine();
        builder.AppendLine("        while (!IsAtEnd && !HasQuoteRun(delimiterWidth))");
        builder.AppendLine("        {");
        builder.AppendLine("            if (Current == '\\r')");
        builder.AppendLine("            {");
        builder.AppendLine("                AdvanceRaw();");
        ...
        builder.AppendLine("    {");
        builder.AppendLine("        if (_index + width > source.Length) return false;");
        builder.AppendLine("        for (var i = 0; i < width; i++)");
        builder.AppendLine("        {");
        builder.AppendLine("            if (source[_index + i] != '\"') return false;");
        builder.AppendLine("        }");
        builder.AppendLine("        return true;");
        builder.AppendLine("    }");
        builder.AppendLine();
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private SollangException Error(string message)");
        builder.AppendLine("    {");
        builder.AppendLine("        return ErrorAt(_line, _column, message);");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static SollangException ErrorAt(int line, int column, string message)");
        builder.AppendLine("    {");
        builder.AppendLine("        return new SollangException($\"lex error at {line}:{column}: {message}\");");
        builder.AppendLine("    }");
        builder.AppendLine("}");
