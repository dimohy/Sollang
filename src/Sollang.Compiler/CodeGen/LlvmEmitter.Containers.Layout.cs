...
internal sealed partial class LlvmEmitter
{
    private string EmitHeapAllocate(string bytes)
    {
        ...
        {
            throw new SollangException("heap allocation is not supported for this target yet");
        }

        var pointer = NextTemp("heap");
        EmitCall(pointer, "ptr", "sollang_alloc", $"i64 {bytes}");
    ...
    private void StoreStaticTextArrayElement(string pointer, int index, RuntimeText value)
    {
        ...
        EmitAssign(slot, $"getelementptr %sollang.text, ptr {pointer}, i64 {index.ToString(CultureInfo.InvariantCulture)}");
        EmitStore("%sollang.text", BuildTextAggregate(value), slot, 8);
    }
    ...
    private RuntimeText EmitStaticTextArrayLoad(RuntimeStaticTextArray array, string index)
    {
        ...
        var slot = NextTemp("text_array_slot");
        EmitAssign(slot, $"getelementptr %sollang.text, ptr {array.PointerName}, i64 {index}");
        var aggregate = NextTemp("text_array_item");
        EmitAssign(aggregate, $"load %sollang.text, ptr {slot}, align 8");
