...
internal sealed partial class LinuxLlvmRuntimePlatform
{
    private const string DnsRuntime = """
        define internal i32 @sollang_dns_linux_error_kind(i32 %code) #0 {
        entry:
          switch i32 %code, label %system [
            i32 -2, label %not_found
            i32 -3, label %temporary
            i32 -6, label %family
            i32 -9, label %family
          ]
        not_found:
        ...
        start_host_copy:
          %native_host = alloca [254 x i8], align 1
          br label %copy_host
        ...
        copy_host_character:
          %host_target = getelementptr i8, ptr %native_host, i64 %host_index
          store i8 %character, ptr %host_target, align 1
        ...
        terminate_host:
          %terminator = getelementptr i8, ptr %native_host, i64 %length
          store i8 0, ptr %terminator, align 1
          br label %prepare_lookup
        ...
        prepare_lookup:
          ...
          %selected_ipv6 = select i1 %is_ipv6, i32 10, i32 0
          ...
          %lookup_code = call i32 @getaddrinfo(ptr %native_host, ptr null, ptr %hints, ptr %results)
          %lookup_ok = icmp eq i32 %lookup_code, 0
          br i1 %lookup_ok, label %begin_results, label %lookup_failed

        lookup_failed:
          %lookup_kind = call i32 @sollang_dns_linux_error_kind(i32 %lookup_code)
        ...
        convert_result:
          %address_pointer = getelementptr i8, ptr %current, i64 24
        ...
        result_limit:
          call void @freeaddrinfo(ptr %first)
