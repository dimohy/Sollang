...
internal static class GrammarCompiler
{
    ...
    public static void Build(string[] args)
    {
        if (args.Length != 4 || args[2] != "-o")
        {
            throw new SollangException(
                "usage: sollang grammar build <lexer-file> <grammar-file> -o <generated.slg>");
        }
    ...
    {
        if (!File.Exists(path)) throw new SollangException($"lexer specification not found: {path}");
        ...
        if (names.Count == 0) throw new SollangException($"{path}: lexer has no token rules");
    ...
    {
        if (!File.Exists(path)) throw new SollangException($"grammar specification not found: {path}");
        var start = "";
        ...
        if (start.Length == 0) throw new SollangException($"{path}: missing start rule");
        if (!seen.Contains(start)) throw new SollangException($"{path}: unknown start rule '{start}'");
        ...
        int stringCount)
    {
        ...
        {
            throw new SollangException("generated grammar has an invalid rule offset table");
        }
        for (var pc = 0; pc < program.Count;)
        {
            ...
            switch (op)
            {
                ...
                case Jump:
                    ...
                    break;
                default:
                    throw new SollangException($"generated grammar has unknown opcode {op}");
            }
        }
    }
        ...
        IReadOnlyList<int> program, ref int pc, Func<int, bool> predicate, string subject)
    {
        ...
        {
            throw new SollangException($"generated grammar has an invalid {subject}");
        }
        pc++;
    }
        ...
        CompiledGrammar grammar, string lexerPath, string grammarPath, string sourceHash)
    {
        ...
        builder.AppendLine("namespace syntax.generated.slg");
        builder.AppendLine();
        builder.AppendLine("# Generated by `sollang grammar build`; do not edit by hand.");
    ...
    private static SollangException SpecError(string path, int line, string message) =>
        ...
        Func<string, int> getStringId)
    {
        ...
        private int RequireToken(string name) => tokenIds.TryGetValue(name, out var id)
            ? id
            : throw new SollangException($"grammar line {line}: unknown token '{name}'");
        ...
        private void Fail(string message) => throw new SollangException($"grammar line {line}: {message}");
    }
}
