...
internal sealed class WindowsLlvmRuntimePlatform : LlvmRuntimePlatform
{
    ...
    public override void EmitComputePrimitives(StringBuilder functions)
    {
        ...
        functions.AppendLine("""
            define internal i32 @smalllang_windows_compute_worker(ptr %unused) #0 {
            entry:
              br label %wait
    ...
    public override void EmitFilePrimitives(StringBuilder functions)
    {
        functions.AppendLine("""
            ...
            define internal %smalllang.file_count_result @smalllang_platform_read_owned_file_at(i64 %handle_value, ptr %data, i64 %len64, i64 %offset) #0 {
            entry:
