...
or stale backlog rows. Current frozen-backlog progress is **fully accepted 4/22
(18.2%); in progress 16/22; blocked 2/22; started 20/22 (90.9%)**. A contract
...
| P1 | Portable memory I/O | Instance readers/writers over caller-owned buffers; partial transfer returns a count without payload allocation; exact transfer is transactional; aggregate helpers require explicit limits | `std.io.MemoryReader.readInto` and `readExactInto` fill caller-owned mutable bytes directly. Exact short reads preserve both cursor and destination, allocating `read`/`readExact` remain conveniences, and fixture 1188 passes managed Windows/Linux partial, over-limit, short, and end paths. The shared protocol will require the caller-buffer partial primitive from each implementation. Deriving transactional exact reads additionally requires a checkpoint capability or an explicitly bounded replay adapter: a generic loop over partial reads cannot roll back a consumed stream or undo bytes already written to the caller. Before any source read, validate the requested length against the scratch/replay ceiling; stage bytes in adapter-owned storage, publish them to the caller only on success, and retain a failed partial prefix for the next logical read. Never describe the underlying file/socket cursor as rolled back. Freeze tests for short input, mid-transfer failure, retry after failure, and pre-read limit rejection before implementing the adapter. The protocol must treat zero progress from a nonempty write as an explicit failure, forbid retaining caller storage, and make every read-all/copy adapter carry byte and scratch-buffer limits. A reusable transfer policy instance will own those limits so copying does not become another ambient global operation. Buffered/reactor adapters later own separate read/write capacities and explicit backpressure; an advisory high-water mark never substitutes for Sollang's checked resource ceiling. Self-host/fixed-point proof, trait-shape fixtures, bounded adapters, and file/socket implementations remain |
| P1 | URI and authority | Parsed components, IPv6 bracket rules, percent codec, and normalization policy without implicit network access | Raw reference parser, typed authority/host/port, and percent codec implemented; normalization/resolution policy next |
| P1 | JSON | Token stream plus typed reader/writer; depth/size limits before allocation; deterministic number contract | Strict token reader and instance-owned writer implemented; typed value mapping and numeric conversion policy remain |
...
| P2 | General algorithms | Sorting, binary search, min/max/clamp, and iterator-style transforms specialized without hidden collection materialization | In progress: allocation-free in-place sorting and an earliest-match/insertion-position search result remain the intended contract. Native direct generic function/inherent-method specialization passes 1437-1441 (5/5); nested helper and recursive concrete calls pass 1448-1449 (2/2). Evidence: `artifacts/scratch/windows-completion/generic-closure-regression-native.log` and `generic-closure-symbolic-native.log`; the former also preserves the failing compiler-as-input 1450 gate. Managed and native 1450 verify five call bindings reuse four concrete bodies. Native dictionary insertion/ownership fixtures 857, 858 and 1451 pass after C2026-09-05-290; evidence: `dictionary-closure-feedback-native.log`, `dictionary-absent-entry-order-native.log` and `dictionary-absent-final-native.log` in the same artifact directory. Dictionary function fixture 66 and native borrowed-container analysis 487 pass after C2026-09-06-291; six E21 negative controls retain external Text origins, including whole-container ownership transfer. Evidence: `scalar-borrow-transfer-native.log` and `scalar-borrow-feedback-native.log` in the same artifact directory. Continue with the instance-first API; grouped lookup performance parity, comparator contracts, other recursive type shapes, algorithm implementation and full acceptance remain unfinished. Do not replace the intended API with public global-function workarounds |
| P2 | Time values | `Instant`, `Duration`, wall clock, monotonic clock, formatting, and injectable clock sources kept distinct | `Duration`, `MonotonicInstant`, `UtcInstant`, effectful marker `MonotonicClock`/`WallClock`, and deterministic mutable manual clocks implemented; legacy `sys.time` and its global aliases removed; calendar formatting and a shared effect-aware source trait remain |
...
| P2 | Text data and paths | CSV records, path component operations, globbing, and regex/search contracts with bounded work and no implicit filesystem access | All eleven natural `sys.path.Path` operations, including effectful `queryRaw`, are zero-wrapper instance methods; their host ABI plus target `nativeStyle` live in `sys/runtime/path.slg`. Only `fromText` construction and target-style selection remain global. Next move the portable value vocabulary to `std.path` while leaving irreducible host primitives in `sys`; keep `normalizeConfined` distinct from symlink-aware canonicalization and reject regex resource exhaustion by construction |
| P2 | Structured logging | Logger instance with typed level/fields and explicit sink; no ambient mutable global logger | Planned after the sink/format contract and disabled-path overhead fixture are fixed |
...
protocols now expose one caller-buffer partial primitive with a fixed
`Result<Int, Error>` progress contract; `MemoryReader` and `MemoryWriter`
...
cross-module generic adapters, bounded replay, read-all/copy policy, file/socket
adapters, and asynchronous buffering still require their own implementation and
platform promotion. The implemented memory protocols and their compiler trait
...
owns an equivalent scheduler/timer runtime. A blocking sleep shim is forbidden.
