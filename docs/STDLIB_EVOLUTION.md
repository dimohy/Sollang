...
(22.7%); in progress 17/22; blocked 0/22; started 22/22 (100%)**. A contract
...
| P2 | General algorithms | Sorting, binary search, min/max/clamp, and iterator-style transforms specialized without hidden collection materialization | In progress: `std.algorithm.Ordering` provides instance-first unstable heapsort, stable bottom-up merge sort, stable sorted two-way merge, first-match/insertion-position binary search, readonly lower/upper/equal-range and sortedness queries, and min/max/clamp under a pure associated-item `Comparison` and explicit direction. `stableSort` mutably borrows caller-owned values plus initialized scratch, preserves both owners, lengths, and capacities, retains equivalent-element order, and returns `StableSortError.InsufficientScratch` before mutation when scratch length is insufficient. `mergeSorted` borrows two already-sorted slices, writes their complete merge into caller-owned preinitialized output, returns the valid prefix length, places equivalent left elements before right elements, and reports length overflow, insufficient capacity, or insufficient visible output length before its first write. Sorting, merge, and value-returning selection remain limited to inline-copyable elements; readonly search supports noncopyable elements through explicit references. `scripts/contracts/general-algorithms.json` and `scripts/verify-general-algorithms-focused.ps1` freeze three positive fixtures (1712, 1713, 1718), three ownership/type negative controls, three generic readonly/reference probes, one stable-sort probe, and one sorted-merge probe covering empty, duplicates, stability, ascending, descending, both short-output failures, input reuse, and output owner/length/capacity preservation. The current managed Windows General focused gate passes 12/12: 11 algorithm cases retain exact execution, authoritative formatting, LLVM assembly, V004 direct-call closure, nonempty library-body inspection, and zero hidden allocation/bulk-copy calls in inspected algorithm bodies, while the twelfth case validates and executes the current-input iterator-transform closure below. The existing `std.sequence` `map`/`mapArray`/`filter`/`tap`/`take`/`skip`/`scan`/`beforeEach`/`afterEach`/`flatMap` APIs require no duplicate iterator API: `scripts/contracts/sequence-iterator-transforms.json` and `scripts/verify-sequence-iterator-transforms-focused.ps1` independently pass 12/12 managed Windows cases covering fused lazy composition, a billion-item range stopped after 54 visits, nested and aggregate state, Text/Bool/UInt64 typed callback paths with no indirect dispatch, first-class and function-boundary streams, shortest-input zip, latest update policy, and merge/latest cancellation. Every native case has exact output, LLVM assembly, and V004 direct-call closure; inspected entry bodies contain no realloc/memcpy/memmove element-materialization calls, local pipelines without source collections contain zero allocator calls, and the `mapArray` composition contains exactly the one allocation written explicitly by its source literal. Bounded first-class producer contexts are permitted and are not element collections. This does not establish selfhost or other-platform acceptance, and noncopyable growable-array transform ownership remains open. Earlier native generic closure and dictionary ownership evidence remains in `artifacts/scratch/windows-completion/generic-closure-regression-native.log`, `generic-closure-symbolic-native.log`, `dictionary-closure-feedback-native.log`, `dictionary-absent-entry-order-native.log`, `dictionary-absent-final-native.log`, `scalar-borrow-transfer-native.log`, and `scalar-borrow-feedback-native.log`. Stable-sort, sorted-merge, and generic readonly/reference selfhost promotion, move-safe noncopyable permutation, output-overlap diagnostics, partition coverage, noncopyable transform coverage, and final Windows/Linux/browser fixed-point verification remain pending. Do not replace the intended API with public global-function workarounds |
...
| P2 | Async task observability | Inspect owned task trees, suspension sites, cancellation, and permanently blocked tasks without changing scheduling semantics | An explicitly enabled affine `DiagnosticSession` now owns bounded task records with stable session/task IDs, parent identity, lifecycle and wait state, awaited-task/timer details, precise UTF-8 suspension locators, caller-owned zero-allocation snapshots, transactional busy close, and consuming terminal close. Structured children inherit the session without an ambient registry, capacity failure preserves the unchanged Task owner and scheduling semantics, and the production-off path emits no diagnostics work beyond the measured two-allocation/two-release async baseline. The current static contract passes 14/14 and the initial managed Windows runtime gate passes 4/4 with exact output, LLVM assembly, direct-call closure, input stability, allocation parity, and zero orphan processes. The remaining managed boundary/negative matrix, current selfhost Windows/Linux execution, browser actionable rejection, and final fixed-point promotion remain before acceptance |
| P2 | Child processes | `sys.process.Command` owns literal executable/argv, environment, directory, and typed `Stdio`; `spawn(move self)` returns an affine `Child`; `wait`, `tryWait`, `kill`, and bounded `collect` expose status, signal, timeout, truncation, and simultaneous stdout/stderr draining; no implicit shell | The owned lifecycle, child-only working directory, instance environment configuration, and directional stdio policy are implemented: `process.command(program)` creates the instance; `arg`/`args`, `workingDirectory`, `setEnvironment`, `removeEnvironment`, and `clearEnvironment` configure it; stdin supports inherit/file/null and stdout/stderr support inherit/file/null without changing parent descriptors. `spawn(move)` returns `Child`; `Child.id` is a zero-wrapper captured `ProcessId`; `wait(move)` reaps to typed `ExitStatus`; scope drop kills then reaps. `Command.collect(CaptureLimits)` overrides stdout/stderr with private pipes, drains both concurrently, retains each stream only to its explicit byte ceiling, and reports independent truncation even at zero limits. Environment inheritance stays explicit, and unchanged inheritance takes the zero-copy parent path. Legacy `run`/`runToFile` route through the same configured primitives. Managed and current Windows self-host fixtures 87, 1132-1133, 1160-1163, and 1179-1183 prove argv, launch failure, explicit wait/drop, directory/environment isolation, file/null stdio, bounded concurrent capture, zero-limit draining, ABI parity, and natural multiline source; Linux fixed-point proof for the newest stdio/capture slice is pending. Executable lookup must not silently reinterpret a relative program against the child directory. Public pipe endpoints remain withheld until `Child` can own their lifetime and backpressure explicitly. `Child.tryWaitTermination(mut)` performs one nonblocking native poll and caches typed ordinary exit or POSIX `SignalStatus` with the exact signal number; repeated typed polls, the compatibility `tryWait`, a later consuming wait, and final drop never reap or close twice. `Child.kill(mut)` requests `TerminateProcess` or `SIGKILL` without consuming or reaping the owner; cached terminal state is an idempotent no-op. Child token/completion fields are private. Managed public Windows fixture 1737 proves live forced termination, retained-owner wait, and cached no-op behavior; fixture 1716, Windows exit-code259 fixture 1717, Linux signal fixture 1757, privacy negatives, and extracted platform-runtime/boundary probes cover nonblocking observation. Linux and current selfhost kill execution remain focused promotion gates. Wait/I/O timeout, typed forced-request attribution, and public pipe handles remain; retain a separately named opt-in shell command only if later justified |
