...
아직은 아주 작습니다. 그래도 SmallLang 코드가 `.wasm`이 되고, 그걸 브라우저가 직접 실행해서 화면에 출력하는 걸 보니… 헐… 재미있네요 ^^;
...
네… 이제 이 출력이 콘솔 exe만이 아니라 브라우저 화면에서도 나옵니다.
...
물론 아직은 작아서 그런 거고, 앞으로 런타임이 커지면 당연히 달라지겠죠. 그래도 첫 브라우저 실행 결과로는 꽤 귀엽습니다… ^^
...
SmallLang이 어디까지 작고 단순하게 갈 수 있을지 계속 사부작 해보겠습니다.
