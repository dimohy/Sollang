안녕하세요. 이번에는 `Sollang`이라는 작은 언어를 사부작 만들고 있습니다.

아직은 정말 작습니다. 언어라고 부르기엔 조금 민망할 정도로요… ^^;

그래도 이제 첫 문법이 조금 더 Sollang다워졌습니다.
...
괄호로 쓰면 `num = square(7)` 같은 형태도 가능하지만, Sollang에서는 일단 "값이 어디로 흘러가는지"가 눈에 보이는 쪽을 선호해보려고 합니다.
...
Roslyn incremental source generator가 `syntax/sollang.lexer`, `syntax/sollang.grammar`를 읽고 `Lexer`, `TokenKind`, `Parser`를 생성합니다.
...
```llvm
define internal i64 @sollang_fn_square(i64 %it) #0 {
entry:
...
```llvm
%call4 = call i64 @sollang_fn_square(i64 7)
```
