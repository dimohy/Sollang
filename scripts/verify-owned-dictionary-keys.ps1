...
& dotnet run --project $runnerProject -c Release --no-build -- `
    ...
    --jobs 2
if ($LASTEXITCODE -ne 0) {
    throw "Owned dictionary key examples failed"
}
...
Write-Host "[owned-keys 3/4] PASS instrumented executables"

Write-Host "[owned-keys 4/4] Detect leaks, double-free, use-after-free, and UB."
foreach ($module in $modules) {
    $actual = Invoke-Wsl @(
        "env",
    ...
    if ($actual -ne $expected) {
        throw "Owned dictionary key mismatch for $($module.Name).`nEXPECTED:`n$expected`nACTUAL:`n$actual"
    }
}
Write-Host "[owned-keys 4/4] PASS owned-key lookup, replacement, take, and drop are sanitizer-clean"
