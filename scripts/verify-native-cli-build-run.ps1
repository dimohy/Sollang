...
Write-Host "[native CLI 3/6] Named-input source build."
...
Write-Host "[native CLI 4/6] Contextual-it source build."
...
Write-Host "[native CLI 5/6] Run and literal argv forwarding."
$runOutput = (& $Compiler run (Join-Path $repoRoot "examples\regression\83-process-arguments.slg") `
    --target windows-x64 --llvm $LlvmHome --stdlib $StdlibRoot -O1 -- `
...
Write-Host "[native CLI 6/6] Complete standard-library linkage."
...
Write-Host "Native explicit-source build/run CLI verification passed (6/6)."
