...
$fixture = "1158-member-array-indexed-member-reference"
...
$notContainsPath = Join-Path $expectedRoot "$fixture.selfhost.llvm.not-contains.txt"
...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "$Label projected-reference verifier input is missing: $requiredPath"
    }
}
...
if ($Platform -eq "windows") {
    $build = Invoke-VerificationProcessCapture `
        ...
        -ArgumentList @(
            ...
            "-O1", "--keep-temps") `
        -Description "$Label projected-reference build"
} else {
    $build = Invoke-VerificationProcessCapture `
        -FilePath "wsl.exe" `
        -ArgumentList @(
...
if ($build.ExitCode -ne 0) {
    throw "$Label projected-reference build failed.`n$($build.Stdout)$($build.Stderr)"
}
if (-not (Test-Path -LiteralPath $executablePath -PathType Leaf) -or
    ...
    throw "$Label projected-reference build did not retain executable and LLVM artifacts"
}
...
foreach ($pattern in ([System.IO.File]::ReadAllLines($orderedRegexPath) |
    ...
    if (-not $match.Success) {
        throw "$Label projected-reference LLVM did not match ordered regex '$pattern' after byte $cursor"
    }
...
foreach ($forbidden in ([System.IO.File]::ReadAllLines($notContainsPath) |
    ...
    if ($llvm.Contains($forbidden)) {
        throw "$Label projected-reference LLVM retained forbidden root-array load '$forbidden'"
    }
}
...
if ($Platform -eq "windows") {
    $run = Invoke-VerificationProcessCapture `
        ...
        -Description "$Label projected-reference executable"
} else {
    $run = Invoke-VerificationProcessCapture `
        -FilePath "wsl.exe" `
...
if ($run.ExitCode -ne 0 -or $actual -cne $expected) {
    throw "$Label projected-reference execution differed.`nExpected:`n$expected`nActual:`n$actual`n$($run.Stderr)"
}
Write-Host "[projected reference] PASS $Label $Platform member/index/member place chain, assembly, and exact execution."
