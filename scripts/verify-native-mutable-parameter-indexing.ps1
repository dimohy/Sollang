...
param(
    ...
    [ValidateSet(
        "1147-mut-parameter-refresh-after-call",
        "1148-second-mut-parameter-refresh-after-call",
        "1149-control-region-mut-parameter-refresh",
        "1150-while-region-mut-parameter-refresh",
        "1151-mutable-parameter-indexing-matrix",
        "1152-early-return-before-checked-index",
        "1153-checked-index-control-before-logical-result",
        "1157-call-wrapped-checked-index-after-early-return"
    )]
    [string]$Fixture = "1147-mut-parameter-refresh-after-call"
)
...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "$Label checked-index regression verifier input is missing: $requiredPath"
    }
}
...
if ($Platform -eq "windows") {
    $build = Invoke-VerificationProcessCapture `
        ...
        -ArgumentList @(
            ...
            "-O1", "--keep-temps") `
        -Description "$Label $Fixture build"
} else {
    $build = Invoke-VerificationProcessCapture `
        -FilePath "wsl.exe" `
        -ArgumentList @(
...
if ($build.ExitCode -ne 0) {
    throw "$Label checked-index regression build failed.`n$($build.Stdout)$($build.Stderr)"
}
if (-not (Test-Path -LiteralPath $executablePath -PathType Leaf) -or
    ...
    throw "$Label checked-index regression build did not retain executable and LLVM artifacts"
}
...
foreach ($contract in ([System.IO.File]::ReadAllLines($regexCountsPath) |
    ...
    if ($fields.Length -ne 2) {
        throw "invalid checked-index regression regex-count contract '$contract'"
    }
    $expectedCount = [int]$fields[0]
    $actualCount = ([regex]::Matches($llvm, $fields[1])).Count
    if ($actualCount -ne $expectedCount) {
        throw "$Label checked-index regression LLVM regex '$($fields[1])' matched $actualCount times instead of $expectedCount"
    }
}
...
foreach ($pattern in ([System.IO.File]::ReadAllLines($orderedRegexPath) |
    ...
    if (-not $match.Success) {
        throw "$Label checked-index regression LLVM did not match ordered regex '$pattern' after byte $cursor"
    }
...
if ($Platform -eq "windows") {
    $run = Invoke-VerificationProcessCapture `
        ...
        -Description "$Label $Fixture executable"
} else {
    $run = Invoke-VerificationProcessCapture `
        -FilePath "wsl.exe" `
...
if ($run.ExitCode -ne 0 -or $actual -cne $expected) {
    throw "$Label checked-index regression execution differed.`nExpected:`n$expected`nActual:`n$actual`n$($run.Stderr)"
}
Write-Host "[checked index regression] PASS $Label $Platform $Fixture ordered bounds and exact execution."
