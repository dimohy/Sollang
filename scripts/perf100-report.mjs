...
const englishLines = [
  ...
  "## What is Perf100?",
  "",
  "Perf100 is a reproducible benchmark suite for evaluating Sollang performance. Its name does not mean 100 unrelated algorithms: it consists of 20 algorithm families, each run with five input profiles, for exactly 100 measured cases. Every case implements the same workload in Sollang, C++, Rust, C# NativeAOT, Go, and Java, verifies byte-identical output, and then compares execution-time ranks under the same conditions. The target is for Sollang to rank in the top three in every case while increasing the number of first-place results.",
  "",
...
const koreanLines = [
  ...
  "## Perf100이란?",
  "",
  "Perf100은 Sollang의 성능을 검증하기 위한 재현 가능한 벤치마크 모음임. 이름의 100은 서로 다른 알고리즘 100개를 뜻하는 것이 아니라, 20개 알고리즘군을 각각 5개 입력 프로필로 실행한 총 100개 측정 사례를 뜻함. 각 사례는 Sollang, C++, Rust, C# NativeAOT, Go, Java에서 동일한 작업을 구현하고, 출력이 바이트 단위로 같은지 확인한 뒤 동일 조건에서 실행 시간 순위를 비교함. 목표는 Sollang이 모든 사례에서 3위 이내를 기록하면서 1위 사례를 확대하는 것임.",
  "",
  "## 측정 계약과 출처",
  "",
...
koreanLines.push(
  ...
  "## 소수 판별 알고리즘 개선",
  "",
  "첫 번째 전체 기준선에서는 단일 배열을 사용하는 가장 큰 소수 판별 사례 하나가 실패 함. 비교 언어들은 저장 공간을 한 번에 할당했지만 Sollang은 확장 가능한 Bool 배열을 수백만 번 push해 초기화하면서 5위가 됨. 따라서 Sollang만 유리하게 바꾸지 않고 여섯 구현 모두를 동일한 분할 체(segmented sieve) 방식으로 변경 함. 이 방식은 32,768개 Int64 원소의 세그먼트, Int64 기반 합성수 표, 동일한 초기화·배수 표시·개수 집계 순서, 변경하지 않은 입력과 체크섬을 사용 함. 최종 전체 측정에서 소수 판별 프로필 5개는 모두 Sollang이 1위를 기록 함.",
  "",
