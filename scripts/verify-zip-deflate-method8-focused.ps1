...
$paths=[ordered]@{compiler=$compiler;verifier=$PSCommandPath;contract=$contractPath;contractSchema=$contractSchemaPath;resultSchema=$resultSchemaPath;resultVerifier=$resultVerifier;fixture=$fixturePath;expected=$expectedPath;oracle=$oraclePath;formatter=$formatVerifier;closure=$closureVerifier;browserRunner=$browserRunner;llvmAs=$llvmAs}
...
$record=[ordered]@{schemaVersion=1;capabilityId='zip-deflate-method8';status='failed';planned=6;completed=0;compiler=[ordered]@{path=$compiler;sha256Start=$null;sha256End=$null};inputs=[ordered]@{start=[ordered]@{};end=[ordered]@{}};windows=[ordered]@{status='not-run';planned=2;completed=0;checks=@((Empty-Check O0),(Empty-Check O2))};linux=[ordered]@{status='not-run';planned=2;completed=0;checks=@((Empty-Check O0),(Empty-Check O2))};browser=[ordered]@{status='not-run';completed=0;artifactProduced=$false;wasmMagicValid=$false;exactOutput=$false;stdoutSha256=$null;stdoutBytes=$null;llvmSha256=$null;bitcodeSha256=$null;artifactSha256=$null;warningCount=$null;noteCount=$null};python=[ordered]@{status='not-run';completed=0;stdoutSha256=$null;stdoutBytes=$null};semanticNegatives=[ordered]@{planned=2;completed=0;failureIds=@()};processAudit=[ordered]@{rootProcessIds=@();descendantProcessIds=@();orphanProcessIds=@();invocations=@()};failureIds=@();startedUtc=[DateTimeOffset]::UtcNow.ToString('O');finishedUtc=[DateTimeOffset]::UtcNow.ToString('O')}
...
try {
    ...
    if($ValidateInputsOnly){$record.status='validated-inputs'}else{
        ...
        foreach($id in $negativeIds){$artifact=Join-Path $output "$id.exe";$negative=@(Compiler-Run "negative-$id" @('build',$paths["negative:$id"],'-o',$artifact,'--target','windows-x64','--llvm',$llvmRoot,'-O0','--keep-temps'))[-1];$diagnostic=[Text.Encoding]::UTF8.GetString($negative.StdoutBytes)+[Text.Encoding]::UTF8.GetString($negative.StderrBytes);$expectedDiagnostic=[IO.File]::ReadAllText($paths["negativeExpected:$id"]).Trim();if($negative.ExitCode-eq 0-or-not$diagnostic.Contains($expectedDiagnostic,[StringComparison]::Ordinal)-or(Test-Path -LiteralPath $artifact)){$record.semanticNegatives.failureIds+=$id;$currentFailureId='SEMANTIC_NEGATIVE_MISMATCH';throw "semantic negative mismatch: $id"};$record.semanticNegatives.completed++}
...
if($record.status-cne'failed'){& $resultVerifier -RepositoryRoot $root -ResultPath (Join-Path $output 'result.json') -ExpectedCompilerSha256 $ExpectedCompilerSha256}
