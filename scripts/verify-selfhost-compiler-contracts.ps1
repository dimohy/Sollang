...
$directOwnedFieldBindingContractPath = Join-Path $RepositoryRoot "scripts/contracts/direct-owned-field-binding.json"
$directOwnedFieldBindingSchemaPath = Join-Path $RepositoryRoot "scripts/contracts/direct-owned-field-binding.schema.json"
$directOwnedFieldBindingResultSchemaPath = Join-Path $RepositoryRoot "scripts/contracts/direct-owned-field-binding-result.schema.json"
$selfhostDirectOwnedFieldBindingResultSchemaPath = Join-Path $RepositoryRoot "scripts/contracts/selfhost-direct-owned-field-binding-result.schema.json"
$directOwnedFieldBindingVerifierPath = Join-Path $RepositoryRoot "scripts/verify-direct-owned-field-binding.ps1"
$selfhostDirectOwnedFieldBindingVerifierPath = Join-Path $RepositoryRoot "scripts/verify-selfhost-direct-owned-field-binding.ps1"
...
$directOwnedFieldBindingContract = [IO.File]::ReadAllText($directOwnedFieldBindingContractPath)
$directOwnedFieldBindingSchema = [IO.File]::ReadAllText($directOwnedFieldBindingSchemaPath)
$directOwnedFieldBindingResultSchema = [IO.File]::ReadAllText($directOwnedFieldBindingResultSchemaPath)
$selfhostDirectOwnedFieldBindingResultSchema = [IO.File]::ReadAllText($selfhostDirectOwnedFieldBindingResultSchemaPath)
$directOwnedFieldBindingVerifier = [IO.File]::ReadAllText($directOwnedFieldBindingVerifierPath)
$selfhostDirectOwnedFieldBindingVerifier = [IO.File]::ReadAllText($selfhostDirectOwnedFieldBindingVerifierPath)
...
Assert-Contains $semanticOwnership 'moveConsumesWholeBinding moveEvent: typedIr.MoveEvent -> Bool' "ownership joins classify canonical whole-binding move sites"
Assert-Matches $semanticOwnership '(?s)moveConsumesWholeBinding moveEvent: typedIr\.MoveEvent -> Bool.*?moveSite\.kind == 6.*?moveSite\.kind == 9.*?moveSite\.kind == 24.*?moveSite\.kind == 25.*?moveSite\.kind == 5.*?moveSite\.typeKind == 4.*?moveSite\.typeFlags / 2 % 2 == 1' "whole-binding move-site contract"
...
Assert-Contains $semanticOwnership 'and (alternativeMove -> moveConsumesWholeBinding)' "ownership joins exclude non-consuming projection events"
...
Assert-Contains $semanticOwnership 'partialMove.memberIr -> placeRootBinding => directFieldOwner' "direct owned-field owner uses canonical place root"
Assert-Contains $semanticOwnership '(directFieldOwner! -> lexicalExecutableAst) == (partialMove.siteIr -> lexicalExecutableAst)' "captured direct owned-field owner is rejected across lexical executables"
Assert-Contains $semanticOwnership 'directFieldOwnerNode.flags % 2 == 1 -> if { true => directFieldAllowed! }' "move input permits direct owned-field transfer"
Assert-Contains $semanticOwnership 'partialMoveIndex! -> immediateInfallibleMemberRepair => directFieldAllowed!' "mutable direct owned-field transfer requires immediate repair"
Assert-Contains $semanticOwnership 'candidateAst.parent == moveAst.parent and candidateAst.start > moveAst.start' "immediate repair uses exact next source statement"
Assert-Matches $semanticOwnership '(?s)assignmentMayExitEarly.*?candidate\.kind == 47 or candidate\.kind == 82' "immediate repair rejects return and result propagation"
Assert-Contains $semanticOwnership 'code: directFieldNeedsRepair! -> if { 36 } else { 34 }' "direct owned-field diagnostics distinguish readonly and missing repair"
Assert-Contains $emitterDiagnostics 'diagnostic.code == 36 -> if {' "mutable direct owned-field repair diagnostic emission"
Assert-Contains $emitterDiagnostics 'must be reinitialized by the immediately following infallible statement' "mutable direct owned-field repair diagnostic contract"
Assert-Contains $emitterDiagnostics 'diagnosticCode == 36 -> if { count! + 1 => count! }' "mutable direct owned-field repair diagnostic blocks code generation"
...
Assert-Matches $ast 'tokens\[signatureToken!\]\.kind == grammar\.tokenIdComma\(\) and afterColon!' "generic parameter commas cannot terminate an inherent method receiver header"
...
Assert-Matches $typedFunctionLowering '(?s)prepared\.controlProducerWrapperByAst\[finalProjectedEligibilityRange\.astStart \+ finalProjectedMethod!\.astNode\].*?finalProjectedCall and finalProjectedHasExplicitReceiver and finalProjectedNeedsResolution\s+and not finalProjectedControlProducerWrapper!' "projected receiver resolution excludes ordinary calls and the indexed outer control-producing flow wrapper"
...
Assert-Matches $typedFunctionLowering '(?s)callHasExplicitReceiverSyntax.*?callAst\.kind == 10.*?callAst\.kind == 11.*?parenthesisDepth.*?tokenIdLeftParen.*?parenthesisDepth! == 0.*?tokenIdRightParen.*?tokenIdIdentifier.*?tokenIdDot' "late exact receiver repair accepts flow and the terminal top-level dotted direct invocation, including nested receivers"
Assert-Matches $typedFunctionLowering '(?s)resolvedCallNeedsExactReceiver.*?callHasExplicitReceiverSyntax.*?targetKind == 7.*?targetKind == 31.*?expected.*?linked' "exact receiver retry separates explicit-receiver provisional globals from arity-mismatched methods"
Assert-Contains $typedFunctionLowering 'finalProjectedCall and finalProjectedHasExplicitReceiver and finalProjectedNeedsResolution' "final projected method repair never reinterprets an ordinary global call argument as a receiver"
Assert-Matches $typedFunctionLowering '(?s)finalProjectedCandidateCall! -> resolvedCallRuntimeParameterCount\(prepared\).*?finalProjectedCandidateArgumentCount! == finalProjectedCandidateParameterCount\s+=> finalProjectedCandidateArityMatches!.*?and finalProjectedCandidateArityMatches!' "final projected method replacement requires exact runtime arity"
...
Assert-Matches $semanticResolve '(?s)referenceNameToken! \+ 1 => referenceSuffixToken!.*?nameAst\.firstToken \+ nameAst\.tokenCount => referenceEndToken.*?nameAst\.kind == 15.*?referenceSuffix\.span\.length == 1.*?source -> byte\(referenceSuffix\.span\.start\)\) == 33.*?true => referenceMutableSuffix!.*?candidate\.kind == 9 or candidate\.kind == 61.*?candidate\.flags % 2 == 1.*?referenceMutableSuffix! and not candidateMutableSuffix.*?candidateMutableSuffix and not referenceMutableSuffix!.*?mutableSuffixMismatch -> if \{ false => namesEqual! \}' "name resolution preserves the mutable bang suffix as part of local semantic identity"
Assert-Matches $semanticResolve '(?s)nameAst\.kind == 16.*?nodes\[nameAst\.parent\]\.kind == 11.*?tokens\[nameAst\.firstToken\]\.kind == grammar\.tokenIdIdentifier\(\).*?tokens\[nameAst\.firstToken \+ 1\]\.kind == grammar\.tokenIdDot\(\).*?nameAst\.firstToken => referenceNameToken!.*?true => dottedDirectCallRoot!' "dotted direct call roots enter ordinary lexical name resolution"
Assert-Contains $semanticResolve 'nameAst.kind == 15 or dottedDirectCallRoot! -> if {' "ordinary names and dotted direct roots share one lexical scope resolver"
Assert-Contains $typedOrdinaryFunction 'knownLocalBindingReference! and expression.kind == 16' "ordinary dotted direct roots lower as lexical name values"
Assert-Contains $typedSourceLowering 'knownEntryLocalBindingReference! and entryExpression.kind == 16' "entry dotted direct roots lower as lexical name values"
Assert-Matches $typedFunctionLowering '(?s)repairDirectAsyncAwaitBindings.*?\(call\.flags / 8\) % 2 == 1.*?callAst\.kind == 10.*?callAst\.firstToken == awaitFlowAst\.firstToken.*?sourceMatches\(awaitToken\.span\.start, awaitToken\.span\.length, "await"\).*?kind: 5.*?operand0: callIndex.*?awaitValueIndex => binding!\.operand0' "direct async await creates one pre-LLVM value edge from the exact Task-producing call"
Assert-Contains $typedResolvedContextFinalize 'nodes -> repairDirectAsyncAwaitBindings(prepared)' "resolved Typed IR finalization repairs direct async await before LLVM"
Assert-Matches $textContext '(?s)awaitSourceNode\.parent >= 0.*?nodes\[awaitSourceRange\.astStart \+ awaitSourceNode\.parent\]\.kind == 10.*?awaitSourceNode\.firstToken == prepared\.package\.nodes\[awaitSourceRange\.astStart \+ awaitSourceNode\.parent\]\.firstToken' "await source indexing admits the exact direct child expression rather than names only"
...
Assert-Contains $calls 'callLeftParenToken! - 1 => exactCallNameToken!' "dotted direct call selects the identifier immediately before its argument list"
Assert-Matches $calls '(?s)callLeftParenToken! - 1\]\.kind == grammar\.tokenIdIdentifier\(\).*?callLeftParenToken! - 2\]\.kind == grammar\.tokenIdDot\(\).*?true => dottedDirectReceiver!' "dotted direct call uses exact terminal identifier and dot syntax"
Assert-Matches $calls '(?s)localCallNode\.kind == 10\s+or localCallNode\.kind == 48\s+or dottedDirectReceiver!' "flow, block-method, and dotted direct calls share exact receiver resolution"
Assert-Matches $calls '(?s)importedIndex! >= 0\s+and dottedDirectReceiver!\s+and qualifiedResults\[importedIndex!\]\.status != 0\s+=> unresolvedDottedReceiver' "unresolved dotted direct paths are classified before exact receiver lookup"
Assert-Matches $calls '(?s)false => qualifiedInherentBlockMethod!.*?importedIndex! >= 0\s+and localCallNode\.kind == 48\s+and qualifiedResults\[importedIndex!\]\.status == 0.*?package\.symbols\[qualifiedBlockTargetRange\.symbolStart \+ qualifiedBlockTarget\.targetSymbol\]\.kind == 31\s+-> if \{ true => qualifiedInherentBlockMethod! \}' "qualified block-method classification requires a resolved function target"
Assert-Matches $calls '(?s)importedIndex! >= 0\s+and not unresolvedDottedReceiver\s+and not qualifiedInherentBlockMethod!\s+=> useQualifiedCall' "qualified flow and resolved direct paths keep qualified resolution outside receiver-owned block dispatch"
Assert-Contains $calls 'useQualifiedCall -> if {' "qualified call classification owns its existing resolution branch"
Assert-Matches $calls '(?s)qualifiedInherentBlockMethod! -> if \{.*?resolvedTargetSource! != qualifiedBlockTargetSource\s+or resolvedFunctionSymbol! != qualifiedBlockTarget\.targetSymbol\s+-> if \{.*?7 => resolvedStatus!' "qualified block methods preserve exact runtime receiver owner and target identity"
...
Assert-Matches $control '(?s)regionReturns regionIndex:.*?regionIndex -> terminatingControlReturns\(context, state\)\s+-> if \{ true => returns! \}' "a direct source-final all-return control terminates its function body without an unmaterialized merge SSA result"
...
Assert-Contains $stage1GenerationSchema '"focusedFixtureRoots": {' "Stage1 generation focused source roots provenance"
Assert-Contains $stage1GenerationSchema '"focusedFixtureRootSha256": {' "Stage1 generation focused source root hashes"
Assert-Contains $stage1GenerationSchema '"focusedFixtureFingerprint": {' "Stage1 generation expanded focused source fingerprint"
Assert-Contains $stage1GenerationSchema '"focusedExpectedPath": {' "Stage1 generation focused expected path provenance"
Assert-Contains $stage1GenerationSchema '"focusedExpectedSha256": {' "Stage1 generation focused expected hash provenance"
Assert-Contains $incrementalVerifier 'focusedFixtureRootSha256 = @($fixtureRoots | ForEach-Object {' "incremental generation records exact focused root hashes"
Assert-Contains $incrementalVerifier 'focusedFixtureFingerprint = $fixtureHash' "incremental generation records expanded focused source fingerprint"
Assert-Contains $incrementalVerifier 'focusedExpectedSha256 = (Get-FileHash -LiteralPath $expectedPath -Algorithm SHA256).Hash' "incremental generation records focused expected hash"
Assert-Contains $directOwnedFieldBindingContract '"id": "P3"' "C341 fixed-storage direct-binding copy positive"
Assert-Contains $directOwnedFieldBindingContract '"source": "examples/regression/1746-fixed-field-direct-binding-copy.slg"' "C341 fixed-storage source-reuse fixture"
Assert-Contains $directOwnedFieldBindingContract '"id": "N2B"' "C341 symmetric child-after-parent overlap negative"
Assert-Contains $directOwnedFieldBindingContract '"source": "examples/regression/diagnostics/direct-owned-field-binding-child-after-parent.slg"' "C341 child-after-parent source fixture"
Assert-Contains $directOwnedFieldBindingSchema '"minItems": 20' "C341 exact contract lower case count"
Assert-Contains $directOwnedFieldBindingSchema '"maxItems": 20' "C341 exact contract upper case count"
Assert-Contains $directOwnedFieldBindingResultSchema '"warningNoteFree"' "managed C341 warning/note-free outcome schema"
Assert-Contains $selfhostDirectOwnedFieldBindingResultSchema '"diagnosticCount"' "selfhost C341 exact diagnostic count schema"
Assert-Contains $selfhostDirectOwnedFieldBindingResultSchema '"warningNoteFree"' "selfhost C341 warning/note-free outcome schema"
Assert-Contains $directOwnedFieldBindingVerifier 'compile produced warning or note:' "managed C341 rejects warning/note output"
Assert-Contains $selfhostDirectOwnedFieldBindingVerifier '1740-selfhost-direct-owned-field-analysis.sources.txt' "selfhost C341 expanded 1740 source-set binding"
Assert-Contains $selfhostDirectOwnedFieldBindingVerifier '$generation.focusedFixtureFingerprint -cne $analysisFingerprint' "selfhost C341 exact 1740 source fingerprint gate"
Assert-Contains $selfhostDirectOwnedFieldBindingVerifier '$generation.focusedExpectedSha256 -cne (Hash $analysisExpectedPath)' "selfhost C341 exact 1740 expected hash gate"
Assert-Contains $selfhostDirectOwnedFieldBindingVerifier '$item.diagnosticCount -ne 1' "selfhost C341 exact negative diagnostic count"
Assert-Contains $selfhostDirectOwnedFieldBindingVerifier '$item.commentOnlyDiagnostic' "selfhost C341 comment-only diagnostic output gate"
Assert-Contains $selfhostDirectOwnedFieldBindingVerifier 'negative produced a warning or note beside the expected error' "selfhost C341 rejects extra warning/note output"
...
Assert-Contains $detachedVerification '"C341Focused" { Join-Path $PSScriptRoot "verify-selfhost-direct-owned-field-binding.ps1" }' "detached C341 focused routing"
Assert-Contains $detachedVerification '"-Stage1Optimization", $IncrementalStage1Optimization' "detached incremental Stage1 optimization routing"
...
Assert-Contains $detachedProgress '"c341focused" { "c341" }' "detached C341 progress stage mapping"
Assert-Contains $detachedProgress '"c341focused" { 20 }' "detached C341 measured denominator"
...
Assert-Contains $detachedResultSchema '"C341Focused"' "detached C341 result schema"
