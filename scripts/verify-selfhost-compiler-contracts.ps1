...
Assert-Matches $calls '(?s)localCallNode\.kind == 10\s+or dottedDirectReceiver!' "flow and dotted direct calls share exact receiver resolution"
...
Assert-Contains $calls 'importedIndex! >= 0 and not unresolvedDottedReceiver => useQualifiedCall' "qualified flow and resolved direct paths keep qualified resolution"
