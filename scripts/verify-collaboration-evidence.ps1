...
function Check-Control {
    ...
    $actualPass = $true
    try { & $Action } catch { $actualPass = $false }
    if ($actualPass -ne $ExpectedPass) { throw "collaboration evidence control failed: $Name" }
    $script:passed++
...
Write-Host "[collaboration evidence] PASS $passed/11; synthetic controls, not a session-completion claim."
